...
public:
    ...
    QList<DATATYPE> search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], t_resultCallback a_resultCallback, void* a_context);
...
protected:
    ...
    bool search(Node* a_node, Rect* a_rect, QList<DATATYPE>& foundItems, t_resultCallback a_resultCallback, void* a_context);
...
RTREE_TEMPLATE
QList<DATATYPE> RTREE_QUAL::search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], t_resultCallback a_resultCallback, void* a_context)
{
#ifdef _DEBUG
    for(int index=0; index<NUMDIMS; ++index)
    {
    ...
    QList<DATATYPE> foundItems;
    search(m_root, &rect, foundItems, a_resultCallback, a_context);
...
RTREE_TEMPLATE
bool RTREE_QUAL::search(Node* a_node, Rect* a_rect, QList<DATATYPE>& foundItems, t_resultCallback a_resultCallback, void* a_context)
{
    ASSERT(a_node);
    ASSERT(a_node->m_level >= 0);
    ASSERT(a_rect);
    ...
    if(a_node->IsInternalNode())
    {
        ...
        for(int index=0; index < a_node->m_count; ++index)
        {
            if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
            {
                if(!search(a_node->m_branch[index].m_child, a_rect, foundItems, a_resultCallback, a_context))
                {
